...
.PHONY: build deploy down logs ps verify test typecheck check-secrets check-updates certs certs-staging preview stage-shared
...
# ha-broker is a standalone package (own lockfile, Docker context = ha-broker/), so it can't import
# the monorepo's @zoci/shared. Rather than hand-maintain a copy of the shared metrics registry, the
# build STAGES it in: one source of truth (shared/src/metrics.ts); ha-broker/src/metrics.ts is a
# generated, gitignored artifact regenerated on every build/test.
stage-shared:     ## Stage shared/src/metrics.ts into the standalone ha-broker build
	@printf '// GENERATED by `make stage-shared` from shared/src/metrics.ts - do not edit.\n' > ha-broker/src/metrics.ts
	@cat shared/src/metrics.ts >> ha-broker/src/metrics.ts

build: stage-shared  ## Build the web + api images
	$(COMPOSE) build
...
test: stage-shared  ## Secret scan + every component's fast unit/contract suite (the deploy gate)
	bash scripts/check-secrets.sh
	npm test
...
test-%: stage-shared  ## One target: api | app | shared | ha-broker | e2e | infra | secrets
	@case '$*' in \
